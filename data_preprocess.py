...
for column in columns_to_change:
    ...
    old_df.to_csv("./data/new_transE_3.csv", index=False)
...
df_12 = pd.read_csv("./data/new_transE_3.csv", usecols=['Item_id','POI_Type','stars','L2_Category_name','clusters'], encoding= 'unicode_escape')
df_12 = df_12.drop_duplicates()
df_12.to_csv("./data/dict.csv", index=False)
...
L2_dict = dict()
df = pd.read_csv('./data/new_transE_3.csv', usecols=['L1_Category','L2_Category_name']) 
...
with open('./data/L2.json', 'w') as json_file:
    json.dump(L2_dict, json_file)

poi_dict = dict()
star_list = dict()
df = pd.read_csv('./data/new_transE_3.csv', usecols=['Item_id','Location_id','POI_Type','POI','stars','L2_Category_name']) 
for index, row in df.iterrows():
...
with open('./data/poi.json', 'w') as json_file:
